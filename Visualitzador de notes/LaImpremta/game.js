/* ============================================================
   LA IMPREMTA CLÀSSICA · TIPOGRAFIA DEL SOLFEIG
   Lògica de joc · Vanilla JS
   ============================================================ */

const NOTES = [
  { name: "DO",  key: "a", color: "#a8201a" },
  { name: "RE",  key: "s", color: "#c9651b" },
  { name: "MI",  key: "d", color: "#c9a44b" },
  { name: "FA",  key: "f", color: "#3a5a3a" },
  { name: "SOL", key: "g", color: "#1a4a6a" },
  { name: "LA",  key: "h", color: "#5a3a8a" },
  { name: "SI",  key: "j", color: "#8a2a5a" }
];
...
// Entrada per ESPAIS (FA·LA·DO5) → LÍNIES (DO·MI·SOL) → escala completa → aguts fins LA5.
const STAGE1 = [    // ESPAIS: FA (e1) + LA (e2) + DO5 (e3)
  { name: "FA",  y: 55 },
  { name: "LA",  y: 45 },
  { name: "DO",  y: 35 }
];
const STAGE2 = [    // LÍNIES: DO ledger (l0) + MI (l1) + SOL (l2)
  { name: "DO",  y: 70, ledger: true },
  { name: "MI",  y: 60 },
  { name: "SOL", y: 50 }
];
const STAGE3 = [    // escala completa de Do Major
  { name: "DO",  y: 70, ledger: true },
  { name: "RE",  y: 65 },
  { name: "MI",  y: 60 },
  { name: "FA",  y: 55 },
...
function makeLevel(n) {
  ...
  if (n <= 4) {
    cfg.tier = "easy"; cfg.notes = STAGE1.slice();        // FA·LA·DO5 (espais)
    cfg.speed = 80 + n * 14; cfg.spawn = 3200 - n * 180; cfg.target = 8 + n;
  } else if (n <= 8) {
    cfg.tier = "easy"; cfg.notes = STAGE2.slice();        // DO·MI·SOL (línies)
    const k = n - 4;
...
const SAVE_KEY = "la-impremta:progress";
...
// FONS · taques de tinta i partícules de paper
(function initInk() {
  ...
  function resize() {
    ...
    parts = Array(40).fill(0).map(() => ({
      ...
      hue: Math.random() < 0.6 ? "#3a2818" : "#5a3a22"
    }));
  }
  resize();
  ...
  function draw() {
    ...
    ctx.fillStyle = "rgba(245,232,200,0.04)";
    ...
    for (const p of parts) {
      ...
      ctx.fillStyle = p.hue + Math.round(p.a * 255).toString(16).padStart(2, "0");
      ctx.beginPath();
...
function playNoteTone(noteName, y) {
  ...
  // timbre clavecí/llaüt
  [{ type: "triangle", vol: 0.15, det: 0 }, { type: "sine", vol: 0.07, det: -7 }].forEach(p => {
    const o = ctx.createOscillator();
    const g = ctx.createGain();
    o.type = p.type;
    o.frequency.value = f;
    ...
    g.gain.setValueAtTime(0, t0);
    g.gain.linearRampToValueAtTime(p.vol, t0 + 0.01);
    g.gain.exponentialRampToValueAtTime(0.0001, t0 + 0.8);
    o.connect(g).connect(sfxGain);
    o.start(t0); o.stop(t0 + 0.85);
  });
}
// SFX impremta: cop fusta + clic metàl·lic / trencament paper
function sndCorrect() {
  // cop sec de fusta
  beep({ freq: 180, dur: 0.06, type: "square", vol: 0.10, slide: -40 });
  beep({ freq: 600, dur: 0.04, type: "triangle", vol: 0.05, delay: 0.01 });
  beep({ freq: 1100, dur: 0.03, type: "sine", vol: 0.04, delay: 0.04 });
}
function sndMiss() {
  // trencament paper + esquitx
  beep({ freq: 800, dur: 0.04, type: "sawtooth", vol: 0.08, slide: 200 });
  beep({ freq: 400, dur: 0.06, type: "sawtooth", vol: 0.07, slide: -200, delay: 0.04 });
  beep({ freq: 200, dur: 0.10, type: "square", vol: 0.06, slide: -80, delay: 0.10 });
}
function sndClick() { beep({ freq: 800, dur: 0.04, type: "triangle", vol: 0.05 }); }
function sndWin() { [523, 659, 784, 1046].forEach((f, i) => beep({ freq: f, dur: 0.20, type: "triangle", vol: 0.08, delay: i * 0.13 })); }
function sndLose() { [330, 277, 220, 165].forEach((f, i) => beep({ freq: f, dur: 0.25, type: "sawtooth", vol: 0.10, delay: i * 0.13 })); }
function sndAch() { [784, 988, 1175, 1568].forEach((f, i) => beep({ freq: f, dur: 0.18, type: "triangle", vol: 0.08, delay: i * 0.10 })); }

// MÚSICA · renaixentista BPM 88 (mode eòlic)
const MUSIC_BPM = 88;
const MUSIC_CHORDS = [
...
function playMusicBar(when, bar, intensity) {
  ...
  c.pad.forEach(f => {
    ...
    g.gain.linearRampToValueAtTime(0.022, when + 0.5);
    g.gain.linearRampToValueAtTime(0.018, when + beat * 4 - 0.5);
  ...
  for (let b = 0; b < 4; b++) {
    ...
    g.gain.exponentialRampToValueAtTime(0.001, when + b * beat + beat * 0.85);
  ...
  for (let s = 0; s < 8; s++) {
    ...
    arp(f, when + s * beat / 2, beat / 4, intensity >= 1.3 ? 0.04 : 0.028);
  }
}
function kick(when) {
  const ctx = audio();
...
// BLOC TIPOGRÀFIC · render variants
const BLOCK_TYPES = ["A", "B", "C", "D", "E"];
function renderBlock(type) {
  const palette = {
    A: { ink: "#a8201a", letter: "𝔄" },
    B: { ink: "#1a3a6a", letter: "𝔅" },
    C: { ink: "#3a5a3a", letter: "ℭ" },
    D: { ink: "#5a3a8a", letter: "𝔇" },
    E: { ink: "#c9a44b", letter: "𝔈" }
  };
  const p = palette[type] || palette.A;
  return `<svg viewBox="0 0 76 38" xmlns="http://www.w3.org/2000/svg">
    <defs>
      <linearGradient id="lead-${type}" x1="0" y1="0" x2="1" y2="1">
    ...
    <!-- ombra projectada -->
    <ellipse cx="38" cy="36" rx="22" ry="2" fill="#000" opacity=".4"/>
    <!-- bloc principal de plom -->
    <rect x="14" y="6" width="48" height="28" rx="1" fill="url(#lead-${type})" stroke="#1a1a1a" stroke-width="0.8"/>
    <!-- bisells: cantonades -->
    <polygon points="14,6 18,2 62,2 62,6" fill="#aaa" opacity=".4"/>
    <polygon points="62,6 62,34 66,30 66,2" fill="#222" opacity=".5"/>
    <!-- cara de la lletra (revers tacat de tinta) -->
    <rect x="18" y="9" width="40" height="22" rx="0.5" fill="#1a0e08"/>
    <!-- taques de tinta a la cara -->
    <ellipse cx="22" cy="11" rx="1.5" ry="0.8" fill="${p.ink}" opacity=".7"/>
    <ellipse cx="55" cy="29" rx="1.2" ry="0.6" fill="${p.ink}" opacity=".6"/>
    <!-- lletra -->
    <text x="38" y="26" text-anchor="middle" font-size="22" font-family="serif" font-weight="900" fill="${p.ink}">${p.letter}</text>
    <!-- reflexos metàl·lics -->
    <rect x="14" y="6" width="48" height="2" fill="#fff" opacity=".25"/>
    <rect x="14" y="32" width="48" height="2" fill="#000" opacity=".4"/>
  </svg>`;
}

// MESTRE GUTENBERG (impressor)
function coachSVG() {
  return `<svg viewBox="0 0 100 140" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMidYMid meet">
    ...
    <!-- cames -->
    <rect x="42" y="95" width="6" height="35" rx="2" fill="#3a2818"/>
    <rect x="52" y="95" width="6" height="35" rx="2" fill="#3a2818"/>
    <rect x="40" y="128" width="10" height="6" rx="1" fill="#1a0e08"/>
    <rect x="50" y="128" width="10" height="6" rx="1" fill="#1a0e08"/>
    <!-- mandil de cuir marró -->
    <path d="M30 50 Q30 45 38 45 H62 Q70 45 70 50 V108 Q70 110 64 110 H36 Q30 110 30 108 Z" fill="#5a3a22" stroke="#3a2818" stroke-width="0.8"/>
    <!-- butxaca pit amb tinta -->
    <rect x="40" y="62" width="12" height="10" fill="none" stroke="#3a2818" stroke-width="0.6"/>
    <ellipse cx="46" cy="60" rx="3" ry="2" fill="#1a0e08"/>
    <text x="50" y="86" text-anchor="middle" font-size="9" font-weight="900" fill="#c9a44b" font-family="serif">𝒢</text>
    <!-- camisa blanca al coll -->
    <path d="M40 45 L50 55 L60 45 Z" fill="#f5e8c8" stroke="#3a2818" stroke-width="0.4"/>
    <!-- braços -->
    <path d="M30 55 Q22 65 24 80 Q26 92 28 96" stroke="#5a3a22" stroke-width="9" fill="none" stroke-linecap="round"/>
    <path d="M70 55 Q78 65 76 80 Q74 90 76 92" stroke="#5a3a22" stroke-width="9" fill="none" stroke-linecap="round"/>
    <!-- mans + pinces tipogràfiques -->
    <circle cx="24" cy="96" r="4" fill="#e8b78a"/>
    <circle cx="76" cy="92" r="4" fill="#e8b78a"/>
    <rect x="73" y="92" width="8" height="3" fill="#888" stroke="#1a1a1a" stroke-width="0.4"/>
    <!-- cap -->
    <circle cx="50" cy="32" r="14" fill="#e8b78a"/>
    <!-- cabells canosos -->
    <path d="M36 28 Q50 14 64 28 Q60 18 50 16 Q40 18 36 28" fill="#aaa"/>
    <!-- barba -->
    <path d="M42 38 Q50 48 58 38 Q56 44 50 46 Q44 44 42 38" fill="#999"/>
    <!-- ulls -->
    <circle cx="44" cy="32" r="1.2" fill="#1a0e08"/>
    <circle cx="56" cy="32" r="1.2" fill="#1a0e08"/>
    <!-- ulleres rodones -->
    <circle cx="44" cy="32" r="3.5" fill="none" stroke="#3a2818" stroke-width="0.6"/>
    <circle cx="56" cy="32" r="3.5" fill="none" stroke="#3a2818" stroke-width="0.6"/>
    <line x1="47.5" y1="32" x2="52.5" y2="32" stroke="#3a2818" stroke-width="0.5"/>
    <!-- somriure -->
    <path d="M45 39 Q50 41 55 39" stroke="#1a0e08" stroke-width="0.8" fill="none" stroke-linecap="round"/>
  </svg>`;
}
...
function renderLevelGrid() {
  ...
  for (let i = 1; i <= 16; i++) {
    ...
    let tier, tag;
    if (i <= 4)       { tier = "easy"; tag = "FA·LA·DO+"; }
    else if (i <= 8)  { tier = "easy"; tag = "DO·MI·SOL"; }
...
function renderPlateHTML(noteY, withLedger, blockType, noteName, forceHint) {
  const Y = noteY + 2;
  const ledger = withLedger
    ? `<line x1="72" y1="${Y}" x2="96" y2="${Y}" stroke="#5a3a22" stroke-width="1.6" stroke-linecap="round"/>`
    : "";
  const hint = forceHint ? `<div class="note-hint">${noteName}</div>` : "";
  return `
    <div class="plate-base"></div>
    <div class="sushi-piece block-${blockType}">${renderBlock(blockType)}</div>
    <div class="staff-card">
      <svg class="staff-svg" viewBox="0 0 116 82" xmlns="http://www.w3.org/2000/svg">
        <line x1="8" y1="22" x2="108" y2="22" stroke="#5a3a22" stroke-width="1.4"/>
        <line x1="8" y1="32" x2="108" y2="32" stroke="#5a3a22" stroke-width="1.4"/>
        <line x1="8" y1="42" x2="108" y2="42" stroke="#5a3a22" stroke-width="1.4"/>
        <line x1="8" y1="52" x2="108" y2="52" stroke="#5a3a22" stroke-width="1.4"/>
        <line x1="8" y1="62" x2="108" y2="62" stroke="#5a3a22" stroke-width="1.4"/>
...
function spawnParticles(x, y, type, color) {
  const count = type === "combo" ? 30 : type === "hit" ? 16 : 12;
  for (let i = 0; i < count; i++) {
    const angle = type === "miss" ? Math.PI + (Math.random() - 0.5) * 0.7 : Math.random() * Math.PI * 2;
    const speed = type === "hit" ? 100 + Math.random() * 180
                : type === "combo" ? 80 + Math.random() * 200
                : 50 + Math.random() * 120;
    particles.push({
      x, y,
      vx: Math.cos(angle) * speed,
      vy: Math.sin(angle) * speed - (type === "hit" ? 60 : type === "miss" ? -40 : 0),
      life: 1,
      decay: type === "miss" ? 1.4 : type === "combo" ? 0.7 : 1.0,
      color: color || (type === "miss" ? "#1a0e08" : type === "combo" ? "#c9a44b" : "#5a3a22"),
      size: type === "combo" ? 3 + Math.random() * 4 : 2 + Math.random() * 3,
      gravity: type === "miss" ? 320 : 180
    });
  }
}
...
function countdownThenStart() {
  ...
  function tick() {
    ...
    beep({ freq: i === seq.length - 1 ? 880 : 440, dur: 0.15, type: "triangle", vol: 0.1 });
    i++;
...
function spawnPlate() {
  ...
  const block = BLOCK_TYPES[(Math.random() * BLOCK_TYPES.length) | 0];
  const el = document.createElement("div");
  el.className = "plate";
  el.style.transform = `translate(-200px, -50%)`;
  el.innerHTML = renderPlateHTML(n.y, !!n.ledger, block, n.name);
...
const ACHIEVEMENTS = [
  { id: "first-folio",  name: "Primera lletra impresa",     test: () => state && state.completed >= 1 },
  { id: "combo-10",     name: "Combo x10 · Caligraf",       test: () => state && state.combo >= 10 },
  { id: "combo-20",     name: "Combo x20 · Tipògraf",       test: () => state && state.combo >= 20 },
  { id: "perfect-stage",name: "Tirada neta · Sense errors", test: () => state && state.completed >= state.cfg.target && state.lives === state.cfg.lives },
  { id: "stage-4",      name: "Aprenent diligent",           test: () => progress.scores[4] != null },
  ...
  { id: "stage-16",     name: "Hereu de Gutenberg",          test: () => progress.scores[16] != null },
...
function showAchToast(a) {
  achToast.innerHTML = `<div class="ach-icon">📜</div><div class="ach-body"><div class="ach-title">SEGELL DESBLOCAT</div><div class="ach-name">${a.name}</div></div>`;
...
function endGame(victory) {
  ...
  saveProgress(progress);
  checkAchievements();
  ovTitle.textContent = victory ? "TIRADA IMPRESA!" : "PAPER TRENCAT";
  ovText.textContent = victory
    ? "Has compost tots els folis. El Mestre Gutenberg està orgullós."
    : "El paper s'ha trencat. Repara la premsa i torna-ho a provar.";
  ...
  ovStats.innerHTML = `
    ...
    <div class="ov-stat"><span class="lbl">FOLIS</span><span class="val">${state.completed}/${state.cfg.target}</span></div>
    <div class="ov-stat"><span class="lbl">PLOMES</span><span class="val">${state.lives}</span></div>
...
function quitToMenu() {
  ...
  if (state.running && !confirm("Vols sortir? Es perdrà el progrés de la tirada actual.")) return;
  stopGame();
  go("levels");
}
...
const BOOT_LINES = [
  "[OK] Encenent forja del taller...",
  "[OK] Fonent plom per als blocs",
  "[OK] Esmolant ploma d'oca",
  "[OK] Preparant 7 calaixos tipogràfics",
  "[OK] Verificant tinta · color negre i or",
  "[OK] Tempo · 88 BPM mode eòlic",
  "[OK] Càrrega de manual de solfeig",
  "[--] Esperant ordres del Mestre Gutenberg..."
];
function runBoot() {
  let p = 0, lineIdx = 0;
...
// TUTORIAL
const TUTORIAL = [
  { type: "speak", text: "Bon dia, aprenent! Sóc el <b>Mestre Gutenberg</b>. T'ensenyaré l'art de la tipografia llegint la nota de cada bloc de plom." },
  { type: "speak", text: "Mira els <b>5 regles metàl·lics</b> de la premsa: són el pentagrama. Els blocs es col·loquen sobre o entre regles." },
  { type: "play", note: "FA", y: 55, block: "A", hint: true,
    intro: "Aquest bloc és al <b>primer espai</b> de la premsa, nota <b>FA</b>. Prem la tecla <b>F</b>.",
    success: "Excel·lent! FA és la tecla F. ✓" },
  { type: "play", note: "LA", y: 45, block: "B", hint: true,
    intro: "Ara <b>LA</b>, al <b>segon espai</b>. Prem la tecla <b>H</b>.",
    success: "Així és, LA és la tecla H. ✓" },
  { type: "play", note: "DO", y: 35, block: "C", hint: true,
    intro: "Aquest és <b>DO</b>, al <b>tercer espai</b>. Prem la tecla <b>A</b>.",
    success: "Increïble!" },
  { type: "speak", text: "Si t'equivoques, no hi ha problema!<br/>El bloc parpelleja vermell i pots <b>reintentar</b> fins que caigui a la paperera." },
  { type: "play", note: "MI", y: 60, block: "D", hint: false,
    intro: "Última prova <b>SENSE pista</b>: aquest bloc és a la <b>primera línia</b>. Quina nota?",
    success: "Perfecte! MI és la tecla D." },
  { type: "speak", final: true, text: "Llest! 7 notes, 7 tecles <b>A·S·D·F·G·H·J</b>.<br/>Al taller, tipògraf!" }
];
...
function renderTutStep() {
  ...
  tutBusy = false;
  tutStepLabel.textContent = `FOLI ${tutIdx + 1} / ${TUTORIAL.length}`;
  ...
  if (step.type === "speak") {
    ...
    tutNext.textContent = step.final ? "▶ AL TALLER" : "SEGÜENT ▶";
  } else if (step.type === "play") {
    const el = document.createElement("div");
    el.className = "plate";
    el.innerHTML = renderPlateHTML(step.y, false, step.block || "A", step.note, step.hint);
