/* ============================================================
   ESPRINT MUSICAL · ESTADI DEL SOLFEIG
   Lògica de joc · Vanilla JS
   ============================================================ */

// ----- DEFINICIONS DE NOTES ---------------------------------
const NOTES = [
  { name: "DO",  key: "a", color: "#ff3a2e" },
  { name: "RE",  key: "s", color: "#ff8c2e" },
  { name: "MI",  key: "d", color: "#ffd23f" },
  { name: "FA",  key: "f", color: "#3eb04f" },
  { name: "SOL", key: "g", color: "#2eb8ff" },
  { name: "LA",  key: "h", color: "#7c4ddb" },
  { name: "SI",  key: "j", color: "#e83a8c" }
];
...
// Entrada per LÍNIES (DO·MI·SOL) → ESPAIS (FA·LA·DO5) → escala completa → aguts fins LA5.
const STAGE1 = [    // LÍNIES: DO ledger (l0) + MI (l1) + SOL (l2)
  { name: "DO",  y: 70, ledger: true },
  { name: "MI",  y: 60 },
  { name: "SOL", y: 50 }
];
const STAGE2 = [    // ESPAIS: FA (e1) + LA (e2) + DO5 (e3)
  { name: "FA", y: 55 },
  { name: "LA", y: 45 },
  { name: "DO", y: 35 }
];
const STAGE3 = [    // escala completa de Do major dins l'octava baixa
  { name: "DO",  y: 70, ledger: true },
  { name: "RE",  y: 65 },
  { name: "MI",  y: 60 },
  { name: "FA",  y: 55 },
...
// LOCALSTORAGE
// ============================================================
const SAVE_KEY = "esprint-musical:progress";
...
(function initDust() {
  ...
  function draw() {
    ...
    for (const p of parts) {
      p.x += p.vx; p.y += p.vy;
      if (p.x < -10) { p.x = cv.width + 10; p.y = Math.random() * cv.height; }
...
// SFX temàtics: tret de sortida, xiulet d'àrbitre
function sndCorrect() {
  // Tret de sortida (Bang!)
  beep({ freq: 80, dur: 0.06, type: "square", vol: 0.18, slide: 60 });
  beep({ freq: 1500, dur: 0.04, type: "square", vol: 0.06, delay: 0.03 });
  beep({ freq: 950, dur: 0.10, type: "sine", vol: 0.08, delay: 0.07 });
}
function sndMiss() {
  // Xiulet d'àrbitre
  beep({ freq: 1800, dur: 0.10, type: "sine", vol: 0.10 });
  beep({ freq: 1500, dur: 0.10, type: "sine", vol: 0.10, delay: 0.10 });
  beep({ freq: 1200, dur: 0.18, type: "sine", vol: 0.10, delay: 0.20 });
}
...
// ============================================================
// MÚSICA · loop esportiu/épic (BPM 120)
// ============================================================
const MUSIC_BPM = 120;
const MUSIC_CHORDS = [
  ...
  { pad: [349.23, 440.00, 523.25], bass: 174.61, arp: [349.23, 440, 523.25, 698.46] }, // F
  { pad: [392.00, 493.88, 587.33], bass: 196.00, arp: [392, 493.88, 587.33, 783.99] }  // G
];
let music = null;
function startMusic() {
...
function playMusicBar(when, bar, intensity) {
  ...
  c.pad.forEach(f => {
    ...
    flt.type = "lowpass"; flt.frequency.value = 1400; flt.Q.value = 1;
    o.type = "sawtooth"; o.frequency.value = f / 2;
    g.gain.setValueAtTime(0, when);
    g.gain.linearRampToValueAtTime(0.022, when + 0.4);
    g.gain.linearRampToValueAtTime(0.020, when + beat * 4 - 0.4);
  ...
  for (let b = 0; b < 4; b++) {
    ...
    g.gain.linearRampToValueAtTime(0.07, when + b * beat + 0.005);
  ...
  if (intensity >= 1) for (let b = 0; b < 4; b++) hat(when + (b + 0.5) * beat, 0.03);
  for (let s = 0; s < 8; s++) {
...
// ============================================================
// CORREDORS · render variants
// ============================================================
const RUNNER_TYPES = ["red", "blue", "green", "yellow", "purple"];
function renderRunner(type) {
  const palette = {
    red:    { kit: "#c8332e", trim: "#ffd23f", num: "01" },
    blue:   { kit: "#2a7bff", trim: "#ffffff", num: "07" },
    green:  { kit: "#3eb04f", trim: "#ffd23f", num: "12" },
    yellow: { kit: "#ffd23f", trim: "#1a1a1a", num: "23" },
    purple: { kit: "#7c4ddb", trim: "#ffffff", num: "44" }
  };
  const p = palette[type] || palette.red;
  return `<svg viewBox="0 0 76 38" xmlns="http://www.w3.org/2000/svg">
    <defs>
      <linearGradient id="kit-${type}" x1="0" y1="0" x2="0" y2="1">
        <stop offset="0%" stop-color="${p.kit}"/>
        <stop offset="100%" stop-color="#1a0a02"/>
      </linearGradient>
    </defs>
    <!-- ombra a la pista -->
    <ellipse cx="38" cy="33" rx="20" ry="2.5" fill="#000" opacity=".3"/>
    <!-- cames en posició de sortida -->
    <path d="M28 30 L34 22 L38 30 Z" fill="#3a1d05"/>
    <path d="M40 30 L48 24 L52 30 Z" fill="#3a1d05"/>
    <!-- tors amb dorsal -->
    <rect x="30" y="14" width="20" height="14" rx="3" fill="url(#kit-${type})" stroke="#3a1d05" stroke-width="0.6"/>
    <rect x="34" y="17" width="12" height="8" rx="1" fill="${p.trim}"/>
    <text x="40" y="24" text-anchor="middle" font-size="6" font-weight="900" font-family="Arial" fill="#1a1a1a">${p.num}</text>
    <!-- braços -->
    <path d="M30 18 L24 22 L26 26" stroke="${p.kit}" stroke-width="3" fill="none" stroke-linecap="round"/>
    <path d="M50 18 L56 14 L58 18" stroke="${p.kit}" stroke-width="3" fill="none" stroke-linecap="round"/>
    <!-- cap amb cinta -->
    <circle cx="40" cy="10" r="5" fill="#e8b78a"/>
    <path d="M35 8 Q40 4 45 8" stroke="${p.kit}" stroke-width="1.5" fill="none"/>
    <circle cx="38.5" cy="10" r=".6" fill="#1a1a1a"/>
    <circle cx="41.5" cy="10" r=".6" fill="#1a1a1a"/>
    <!-- línia de motion -->
    <path d="M14 18 L22 18" stroke="#fff" stroke-width="1" opacity=".6"/>
    <path d="M16 22 L24 22" stroke="#fff" stroke-width="0.7" opacity=".4"/>
  </svg>`;
}
...
function coachSVG() {
  return `<svg viewBox="0 0 100 140" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMidYMid meet">
    ...
    <!-- cames -->
    <rect x="42" y="95" width="6" height="35" rx="2" fill="#1a3a1f"/>
    <rect x="52" y="95" width="6" height="35" rx="2" fill="#1a3a1f"/>
    <rect x="40" y="128" width="10" height="6" rx="1" fill="#1a1a1a"/>
    <rect x="50" y="128" width="10" height="6" rx="1" fill="#1a1a1a"/>
    <!-- xandall -->
    <path d="M30 50 Q30 45 38 45 H62 Q70 45 70 50 V100 Q70 105 62 105 H38 Q30 105 30 100 Z" fill="#c8332e" stroke="#7a1d18" stroke-width="0.8"/>
    <!-- ratlla blanca lateral -->
    <rect x="30" y="60" width="40" height="3" fill="#ffd23f"/>
    <text x="50" y="78" text-anchor="middle" font-size="10" font-weight="900" fill="#fff" font-family="Arial">COACH</text>
    <!-- braços -->
    <path d="M30 55 Q22 65 24 80 Q26 90 30 92" stroke="#c8332e" stroke-width="9" fill="none" stroke-linecap="round"/>
    <path d="M70 55 Q78 65 76 80 Q74 88 76 92" stroke="#c8332e" stroke-width="9" fill="none" stroke-linecap="round"/>
    <!-- mans -->
    <circle cx="24" cy="92" r="4" fill="#e8b78a"/>
    <circle cx="76" cy="92" r="4" fill="#e8b78a"/>
    <!-- cronòmetre a la mà dreta -->
    <circle cx="76" cy="98" r="5" fill="#ffd23f" stroke="#1a1a1a" stroke-width="0.8"/>
    <line x1="76" y1="98" x2="76" y2="95" stroke="#1a1a1a" stroke-width="1"/>
    <line x1="76" y1="98" x2="79" y2="98" stroke="#1a1a1a" stroke-width="1"/>
    <!-- cap -->
    <circle cx="50" cy="32" r="14" fill="#e8b78a"/>
    <!-- gorra de cap d'equip -->
    <path d="M36 26 Q36 16 50 16 Q64 16 64 26 L66 30 L34 30 Z" fill="#1a1a1a"/>
    <ellipse cx="50" cy="29" rx="18" ry="3" fill="#1a1a1a"/>
    <text x="50" y="25" text-anchor="middle" font-size="7" font-weight="900" fill="#ffd23f" font-family="Arial">M</text>
    <!-- cara -->
    <circle cx="45" cy="33" r="1.2" fill="#1a1a1a"/>
    <circle cx="55" cy="33" r="1.2" fill="#1a1a1a"/>
    <path d="M45 40 Q50 43 55 40" stroke="#1a1a1a" stroke-width="1.2" fill="none" stroke-linecap="round"/>
    <!-- silbat al coll -->
    <line x1="50" y1="46" x2="50" y2="56" stroke="#fff" stroke-width="1"/>
    <rect x="47" y="55" width="6" height="3" fill="#c0c0c0"/>
  </svg>`;
}
...
// Pinta entrenador SVG a tots els elements .coach-svg
...
function renderLevelGrid() {
  ...
  for (let i = 1; i <= 16; i++) {
    ...
    let tier, tag;
    if (i <= 4)       { tier = "easy"; tag = "DO·MI·SOL"; }
    else if (i <= 8)  { tier = "easy"; tag = "FA·LA·DO+"; }
...
// ============================================================
// PLAT (corredor) · render
// ============================================================
function renderPlateHTML(noteY, withLedger, runnerType, noteName, forceHint) {
  const Y = noteY + 2;
  const ledger = withLedger
    ? `<line x1="72" y1="${Y}" x2="96" y2="${Y}" stroke="#fff" stroke-width="1.4" stroke-linecap="round" stroke-dasharray="3 2"/>`
    : "";
  const hint = forceHint ? `<div class="note-hint">${noteName}</div>` : "";
  return `
    <div class="plate-base"></div>
    <div class="sushi-piece runner-${runnerType}">${renderRunner(runnerType)}</div>
    <div class="staff-card">
      <svg class="staff-svg" viewBox="0 0 116 82" xmlns="http://www.w3.org/2000/svg">
        <line x1="8" y1="22" x2="108" y2="22" stroke="#fff" stroke-width="1.1"/>
        <line x1="8" y1="32" x2="108" y2="32" stroke="#fff" stroke-width="1.1"/>
        <line x1="8" y1="42" x2="108" y2="42" stroke="#fff" stroke-width="1.1"/>
        <line x1="8" y1="52" x2="108" y2="52" stroke="#fff" stroke-width="1.1"/>
        <line x1="8" y1="62" x2="108" y2="62" stroke="#fff" stroke-width="1.1"/>
        <text x="4" y="60" font-size="44" fill="#fff" font-family="serif" font-weight="700">𝄞</text>
        ${ledger}
        <ellipse cx="84" cy="${Y}" rx="7" ry="4.8" fill="#fff" transform="rotate(-18 84 ${Y})"/>
        <ellipse cx="84" cy="${Y}" rx="3.6" ry="2.4" fill="#ffd23f" transform="rotate(-18 84 ${Y})" opacity=".9"/>
      </svg>
      ${hint}
    </div>
  `;
...
function spawnParticles(x, y, type, color) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    particles.push({
      ...
      color: color || (type === "miss" ? "#c8332e" : type === "combo" ? "#ffd23f" : "#3eb04f"),
...
function spawnPlate() {
  ...
  const runner = RUNNER_TYPES[(Math.random() * RUNNER_TYPES.length) | 0];
  const el = document.createElement("div");
  el.className = "plate";
  el.style.transform = `translate(-200px, -50%)`;
  el.innerHTML = renderPlateHTML(n.y, !!n.ledger, runner, n.name);
...
// ACHIEVEMENTS
const ACHIEVEMENTS = [
  { id: "first-runner", name: "Primer dorsal",            test: () => state && state.completed >= 1 },
  { id: "combo-10",     name: "Combo x10 · Pas a pas",    test: () => state && state.combo >= 10 },
  { id: "combo-20",     name: "Combo x20 · Esprintador",  test: () => state && state.combo >= 20 },
  { id: "perfect-stage",name: "Cursa neta · Sense errors",test: () => state && state.completed >= state.cfg.target && state.lives === state.cfg.lives },
  { id: "stage-4",      name: "Mestre dels espais",        test: () => progress.scores[4] != null },
  ...
  { id: "stage-16",     name: "Campió Olímpic",            test: () => progress.scores[16] != null },
...
function showAchToast(a) {
  achToast.innerHTML = `<div class="ach-icon">🏆</div><div class="ach-body"><div class="ach-title">MEDALLA DESBLOCADA</div><div class="ach-name">${a.name}</div></div>`;
...
function endGame(victory) {
  ...
  saveProgress(progress);
  checkAchievements();
  ovTitle.textContent = victory ? "META CREUADA!" : "DESQUALIFICAT";
  ovText.textContent = victory
    ? "Has portat tots els dorsals a la meta. L'entrenador està orgullós."
    : "Has esgotat les energies. Recupera't i torna-ho a provar.";
  ...
  ovStats.innerHTML = `
    ...
    <div class="ov-stat"><span class="lbl">DORSALS</span><span class="val">${state.completed}/${state.cfg.target}</span></div>
    <div class="ov-stat"><span class="lbl">ENERGIES</span><span class="val">${state.lives}</span></div>
...
function quitToMenu() {
  ...
  if (state.running && !confirm("Vols sortir? Es perdrà el progrés de la cursa actual.")) return;
  stopGame();
  go("levels");
}
...
const BOOT_LINES = [
  "[OK] Encenent focus de l'estadi...",
  "[OK] Carregant pista de tartà",
  "[OK] Calibrant cronòmetre digital",
  "[OK] Preparant 7 carrers de sortida",
  "[OK] Sincronitzant el silbat d'àrbitre",
  "[OK] Tempo · 120 BPM",
  "[OK] Càrrega de mòdul solfeig",
  "[--] Esperant senyal de l'entrenador..."
];
function runBoot() {
...
// TUTORIAL
const TUTORIAL = [
  { type: "speak", text: "Bon dia! Soc l'<b>Entrenador Marc</b>. T'ensenyaré a llegir notes a la pista. Au, escalfem!" },
  { type: "speak", text: "Mira el pentagrama: <b>5 línies</b> que són els 5 carrers de la pista. Els corredors es col·loquen sobre la línia o entre carrers." },
  { type: "play", note: "DO", y: 70, runner: "red", hint: true,
    intro: "Aquest corredor té una <b>línia addicional</b> sota el pentagrama: és <b>DO</b>. Prem la tecla <b>A</b>.",
    success: "Excel·lent! DO és la tecla A. ✓" },
  { type: "play", note: "MI", y: 60, runner: "yellow", hint: true,
    intro: "Ara <b>MI</b>, a la <b>primera línia</b>. Prem la tecla <b>D</b>.",
    success: "Així és, MI és la tecla D. ✓" },
  { type: "play", note: "SOL", y: 50, runner: "blue", hint: true,
    intro: "Aquest és <b>SOL</b>, a la <b>segona línia</b>. Prem la tecla <b>G</b>.",
    success: "Increïble!" },
  { type: "speak", text: "Si t'equivoques, no passa res!<br/>El corredor parpelleja vermell i pots <b>reintentar</b> fins que es desqualifiqui." },
  { type: "play", note: "FA", y: 55, runner: "green", hint: false,
    intro: "Última prova <b>SENSE pista</b>: aquest corredor és al <b>primer espai</b>. Quina nota?",
    success: "Perfecte! FA és la tecla F." },
  { type: "speak", final: true, text: "Llest! 7 notes, 7 tecles <b>A·S·D·F·G·H·J</b>.<br/>A la pista, atleta!" }
];
...
function renderTutStep() {
  ...
  if (step.type === "speak") {
    ...
    tutNext.textContent = step.final ? "▶ A LA PISTA" : "SEGÜENT ▶";
  } else if (step.type === "play") {
    const el = document.createElement("div");
    el.className = "plate";
    el.innerHTML = renderPlateHTML(step.y, false, step.runner || "red", step.note, step.hint);
...
// MÚSICA · arrencada al primer click
